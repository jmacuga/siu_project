...
env.PI_BY=3
                                 # pocz. odchyłka od azymutu na cel losowo w przedziale +-pi/3
...
for s in args.model_names:                              # ocena dla każdego modelu - argumentu wywołania
    ...
    collisions=0                                        # liczba kolizji / awarii agenta (done)
    succeeded_targets=0                                 # liczba poprawnie osiągniętych celów pośrednich
    ...
    while step<max_test_steps:
        print(f'step: {step}')                          # jednoczesny krok symulacji wszystkich agentów
        ride+=len(to_restart)
        ...
        for tname,(new_state,reward,done) in scene.items():         # przestawienie zółwi na kolejne cele
            ...
            fds = np.full(rlen, np.inf)                 # odległości od celów; inf = pominięty
            prev_goal = (section_ids[tname] - 1) % rlen
            pose = env.agents[tname].pose
            for goal_id in range(rlen):
                if rlen > 1 and goal_id == prev_goal:   # pomiń poprzednio osiągnięty cel (przy 1 celu na trasę — nie pomijaj)
                    continue
                fds[goal_id] = np.sqrt((route[goal_id][5] - pose.x)**2 + (route[goal_id][6] - pose.y)**2)
    ...
    print(f'kolizje: {collisions}, osiagniete cele: {succeeded_targets}, proby: {ride}, {lap} okrazen, {lap/ride:.2f} okrazen na 1 probe')
    with open(args.res_file,'a') as f:
        f.write(f'{collisions} {succeeded_targets} {ride} {lap} {lap/ride:.2f} {model_name}\n')
