...
class DqnSingle:
    ...
    def __init__(self, env: TurtlesimEnvBase, id_prefix="dqns", seed=42):
        ...
        self.SAVE_MODEL_EVERY = (
            250  # *  co ile epizodów zapisywać model # TODO STUDENCI
        )
        random.seed(seed)
        np.random.seed(seed)
        self.model = None
    ...
    def inp_stack(_, last, cur):
        ...
        inp = np.stack(
            [cur[2], cur[3], cur[4], cur[5], last[2], last[3], last[4], last[5]],
            axis=-1,
        )
        return inp
    ...
    def train_main(
        ...
        self.target_model = keras.models.clone_model(
            self.model
        )  # model pomocniczy (wolnozmienny)
        self.replay_memory = deque(maxlen=self.REPLAY_MEM_SIZE_MAX)  # historia kroków
        episode_rewards = (
            np.zeros(self.EPISODES_MAX) * np.nan
        )  # historia nagród w epizodach
        ...
        for episode in range(self.EPISODES_MAX):  # ucz w epizodach treningowych
            ...
            while True:  # o przerwaniu decyduje do_train()
                if np.random.random() > epsilon:  # sterowanie wg reguły albo losowe
                    control = np.argmax(
                        self.decision(self.model, last_state, current_state)
                    )
                    print("o", end="")  # "o" - sterowanie z modelu
                else:
                    control = np.random.randint(
                        0, self.CTL_DIM
                    )  # losowa prędkość pocz. i skręt
                    print(".", end="")  # "." - sterowanie losowe
                new_state, reward, done = self.env.step(
                    {tname: self.ctl2act(control)}
                )  # krok symulacji
                step_cnt += 1
                episode_rwrd += reward
                self.replay_memory.append(
                    (last_state, current_state, control, reward, new_state, done)
                )
                # bufor ruchów dość duży oraz przyszła pora by podtrenować model
                if (
                    len(self.replay_memory) >= self.REPLAY_MEM_SIZE_MIN
                    and step_cnt % self.TRAIN_EVERY == 0
                ):
